...
for filename in glob.glob('*.nc4'):
    ...
    dn = df[df.index.isin(Index)]
    Daily_avg.append(dn['TWS_tavg'].mean())
daily=[x for x in Daily_avg if pd.isnull(x) == False]
x=(daily[155]+daily[157])/2
daily.insert(156, x)
print(len(daily))
yearly = np.average(daily)

print('terrestrial water storage daily avg is:',daily)
print('terrestrial water storage yearly avg is:', yearly)
plt.plot(daily)
plt.xlabel("Day")
plt.ylabel("Average Storage (mm)")
plt.title('terrestrial water storage year 90')
plt.show()

# ds = nc.Dataset("GLDAS_CLSM025_DA1_D.A20101001.022.nc4.SUB.nc4")
